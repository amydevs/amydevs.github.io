...
async function getStaticProps() {
  ...
  const { base64 } = await plaiceholder.getPlaiceholder(Buffer.from(buffer), {
    size: 10,
  });
      ...
      <main className="flex-1">
        ...
        >
          <Image
            ref={imageRef}
            className={cn(
              "rounded-full blur-none transition-all",
              !imageLoaded && "blur-sm",
            )}
            width={460}
            height={460}
            placeholder={pfpBlurDataURL != null ? "blur" : "empty"}
            blurDataURL={pfpBlurDataURL}
            src={pfpSrc}
            alt="PFP"
          ...
          <div>
            ...
            <span className="text-xl">
              I&apos;m a software engineer and computer science student based in
              Australia.
            </span>
            <br />
          </div>
