...
The regex here simply matches the `src` and `data-canonical-src` attributes of the `<img />` tags on the GitHub profile page. We can do this because GitHub will expand all image blocks to include both the original URL and the Camo CDN URL. Notice that `he.decode` is necessary here, because GitHub may encode characters in the original image URL with HTML entities.